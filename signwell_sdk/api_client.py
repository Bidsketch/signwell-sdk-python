...
from signwell_sdk.api_response import ApiResponse, T as ApiResponseT
import signwell_sdk.models
from signwell_sdk import rest
from signwell_sdk.exceptions import (
    ApiValueError,
    ApiException,
    BadRequestException,
...
class ApiClient:
    ...
    def __init__(self, configuration=None, header_name=None, header_value=None, cookie=None) -> None:
        ...
        self.user_agent = "SignWell-Python-SDK/0.1.0"
    ...
    def close(self):
        """Clear pooled HTTP connections held by the underlying REST client."""
    ...
    def deserialize(self, response_text: str, response_type: str, content_type: str | None):
        ...
        if content_type is None:
            ...
            if response_type not in ("str", "object"):
                raise UnsupportedContentTypeError(status=0, reason=f"Unsupported content type: {content_type}")
            data = response_text
        else:
            raise UnsupportedContentTypeError(status=0, reason=f"Unsupported content type: {content_type}")
    ...
    def __deserialize(self, data, klass):
        ...
        if isinstance(klass, str):
            if klass.startswith("List["):
                ...
                if m is None:
                    raise ApiValueError("Malformed List type definition")
                sub_kls = m.group(1)
            ...
            if klass.startswith("Dict["):
                ...
                if m is None:
                    raise ApiValueError("Malformed Dict type definition")
                sub_kls = m.group(2)
            ...
            if klass in self.NATIVE_TYPES_MAPPING:
                ...
                klass = getattr(signwell_sdk.models, klass)
    ...
    def __deserialize_file(self, response):
        ...
        Saves response body into a file in a temporary folder. A sanitized
        `Content-Disposition` basename may influence the temp file prefix and
        suffix, but never the directory or complete output path.
        ...
        if content_disposition:
            ...
            if m is not None:
                ...
                if filename:
                    ...
                    if filename_prefix:
                        prefix = f"{filename_prefix[:50]}-"
                    if filename_suffix:
                        suffix = filename_suffix[:20]
    ...
    def __sanitize_download_filename(filename):
        """Return a safe basename for a server-supplied download filename."""
        basename = re.split(r"[\\/]", filename)[-1]
        return re.sub(r"[^A-Za-z0-9._-]+", "_", basename).strip("._")
